...
//}
@Singleton
class PhanCongRepository @Inject constructor(
    private val phanCongDao: PhanCongDao,
    private val phanCongKtvDao: PhanCongKtvDao,
    private val thietBiDao: ThietBiDao,
    private val syncService: BaseFirestoreSyncService<PhanCong> // 🔄 Bổ sung sync
) {
    ...
    suspend fun capNhatTrangThaiPhanCong(phanCongId: Int) {
        val dsKtv = phanCongKtvDao.getAllByPhanCongId(phanCongId)
        val allTrangThai = dsKtv.map { it.trangThai }
        val trangThaiCoGiaTri = allTrangThai.filter {
            it != TrangThaiPhanCong.DA_TU_CHOI && it != TrangThaiPhanCong.BI_HUY
        }

        val newTrangThai = when {
            trangThaiCoGiaTri.isEmpty() -> TrangThaiChungCuaPhanCong.CHUA_BAT_DAU
            trangThaiCoGiaTri.all { it == TrangThaiPhanCong.HOAN_THANH } -> TrangThaiChungCuaPhanCong.HOAN_THANH
            trangThaiCoGiaTri.all { it == TrangThaiPhanCong.CHO_PHAN_HOI || it == TrangThaiPhanCong.DA_CHAP_NHAN } ->
                TrangThaiChungCuaPhanCong.CHUA_BAT_DAU
            trangThaiCoGiaTri.any { it == TrangThaiPhanCong.DANG_THUC_HIEN } ->
                TrangThaiChungCuaPhanCong.DANG_THUC_HIEN
            trangThaiCoGiaTri.all {
        ...
        // 🔄 Bổ sung sync trạng thái mới lên Firestore
        pc?.let {
...
//    }
    ...
    suspend fun update(phanCong: PhanCong) {
        ...
        SyncDispatcher.dispatch(syncService, phanCong, SyncDispatcher.SyncType.UPDATE) // 🔄 Bổ sung
    }

    suspend fun delete(phanCong: PhanCong) {
        phanCongDao.delete(phanCong)
        SyncDispatcher.dispatch(syncService, phanCong, SyncDispatcher.SyncType.DELETE) // 🔄 Bổ sung
    }

    suspend fun insertAndGetId(phanCong: PhanCong): Long {
        val id = phanCongDao.insertAndGetId(phanCong)
        SyncDispatcher.dispatch(syncService, phanCong.copy(id = id.toInt()), SyncDispatcher.SyncType.INSERT) // 🔄 Bổ sung sync cho insert có ID
        return id
    }
